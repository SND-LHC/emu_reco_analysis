...
void check_raw( int output, char *fname)
{
  ...
  if (run) printf("run is taken");
  else printf("something else");
  init();
  printf("The script is initialized");
  ...
  if(do_surf)  {
   ...
   if(output==1  || (fname&&output==3) ) {
      ...
      gSystem->Sleep(500);
      if(cz)    cz->SaveAs("raw_z.png");
      if(cview) cview->SaveAs("raw_view.png");
      if(csurf) csurf->SaveAs("raw_surf.png");
    }
   if(run) run->Close();
