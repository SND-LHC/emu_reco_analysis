...
// launch this function after hadd of SIG/BKG OUTvar.root files
void prepareTrees(TString filepath){
	TFile 	*inFile	= 	TFile::Open(filepath);
	if (inFile == NULL){
		std::cout << "ERROR: could not open data file" << std::endl;
     	exit(1);
	}	
	TTree 	*inTree		= 	(TTree*)inFile->Get("vtx");
	TFile	*outFile	= 	TFile::Open("TMVA_in_numu_muon1E5.root","RECREATE");
	TTree	*outTreeS	= 	inTree->CopyTree("signal==1");
	TTree	*outTreeB	=	inTree->CopyTree("signal==0");
	outTreeS->SetObject("TreeS", "Signal TMVA input tree");
	outTreeB->SetObject("TreeB", "Background TMVA input tree");
	outFile->cd();
	outTreeS->Write();
	outTreeB->Write();
}
...
int TMVA_analysis( TString myMethodList = "" )
{
   ...
   // POSSIBLE PRE-CUTS TO BE APPLIED HERE
   ...
   dataloader->AddVariable( "ntrks",		'I');
   dataloader->AddVariable( "prob",			'F');
   dataloader->AddVariable( "meanIP",		'F');
   dataloader->AddVariable( "fillfact",	'F');
   dataloader->AddVariable( "maxdphi",	'F');
   ...
   // 30% train - 70% test for both Sig and Bkg
   dataloader->PrepareTrainingAndTestTree( mycuts, mycutb,
                                        "nTrain_Signal=783:nTrain_Background=1550:SplitMode=Random:NormMode=NumEvents:!V" );
