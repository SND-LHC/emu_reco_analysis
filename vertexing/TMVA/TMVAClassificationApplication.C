...
void TMVAClassificationApplication( TString myMethodList = "" )
{
   ...
   reader->AddVariable( "ntrks", &var1);
   reader->AddVariable( "prob", &var2);
   reader->AddVariable( "meanIP", &var3);
   reader->AddVariable( "fillfact", &var4);
   reader->AddVariable( "maxdphi", &var5);

   // Spectator variables declared in the training have to be added to the reader, too
   //Float_t spec1; Float_t spec2;
   //reader->AddSpectator( "brickID",   &spec1 );
   //reader->AddSpectator( "weight",   &spec2 );

   // Book the MVA methods

   //TString dir    = "/eos/user/v/valbano/vertex/dataset/weights/";

   TString dir    = "/afs/cern.ch/work/f/falicant/public/TMVA/dataset/weights/";
   ...
   //
   TFile *input(0);
   TString fname = "/afs/cern.ch/work/f/falicant/public/RUN3/brick44/57_1/vertex_selection.root";
   ...
   TTree* theTree = (TTree*)input->Get("vtx");
   Int_t int_var1;
   theTree->SetBranchAddress( "ntrks", &int_var1 );
   theTree->SetBranchAddress( "prob", &var2 );
   theTree->SetBranchAddress( "meanIP", &var3 );
   theTree->SetBranchAddress( "fillfact", &var4 );
   theTree->SetBranchAddress( "maxdphi", &var5 );

   
   // TTree* outTree = new TTree("bdtTree", "BDT tree");
   TFile *outputFile  = new TFile( "/afs/cern.ch/work/f/falicant/public/RUN3/brick44/57_1/vertex_TMVA.root","RECREATE" );
   TTree* outTree = theTree->CopyTree("");
   Float_t BDT_sel;
   TBranch* BDT_branch = outTree->Branch("BDT", &BDT_sel, "BDT/F");
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      var1 = int_var1;
      BDT_sel = reader->EvaluateMVA( "BDT method" );
      BDT_branch->Fill();
/*
   Int_t jevt = 0;
      theTree->GetEntry(ievt);
      selTree->GetEntry(jevt);
      if  (var7==quad_sel&&var8==subquad_sel&&var6==vID_sel) {
		BDT_sel = reader->EvaluateMVA( "BDT method" );
...
*/
   ...
   // Write histograms
/*
   TFile *target  = new TFile( "TMVA_daniele.root","RECREATE" );
